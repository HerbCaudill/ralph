...
describe("serverConfig", () => {
  ...
  beforeEach(() => {
    ...
    for (const key of Object.keys(import.meta.env)) {
      if (key.startsWith("VITE_BEADS") || key.startsWith("VITE_AGENT") || key.startsWith("VITE_SPLIT")) {
  ...
  describe("buildServerUrl", () => {
    ...
    describe("split mode via explicit URLs", () => {
      ...
      it("routes unknown paths to agent-server by default", () => {
        expect(buildServerUrl("/api/something-else")).toBe("http://localhost:4244/api/something-else")
      })
    })
  })
})
