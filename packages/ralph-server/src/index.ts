...
// ── Re-exports: adapter modules (from @herbcaudill/agent-server) ─────

export { ClaudeAdapter, buildCwdContext } from "@herbcaudill/agent-server"
export type { ClaudeAdapterOptions, QueryFn, RetryConfig } from "@herbcaudill/agent-server"

export { CodexAdapter } from "@herbcaudill/agent-server"
export type { CodexAdapterOptions, CodexFactory } from "@herbcaudill/agent-server"
...
// ── Re-exports: session management (from @herbcaudill/agent-server) ──

export { SessionPersister, ChatSessionManager } from "@herbcaudill/agent-server"
export type {
  SessionInfo,
...
export { registerRoutes as registerGenericRoutes } from "@herbcaudill/agent-server"
export type { RouteContext as GenericRouteContext } from "@herbcaudill/agent-server"
export { handleWsConnection as handleGenericWsConnection } from "@herbcaudill/agent-server"
export type {
  WsClient as GenericWsClient,
  WsHandlerOptions as GenericWsHandlerOptions,
} from "@herbcaudill/agent-server"

// ── Re-exports: utility functions (from @herbcaudill/agent-server) ───

export { isRetryableError } from "@herbcaudill/agent-server"
export { calculateBackoffDelay } from "@herbcaudill/agent-server"
export { createEventStream } from "@herbcaudill/agent-server"
export { createMessageStream } from "@herbcaudill/agent-server"
...
/**
 * Read ralph server configuration from environment variables.
 */
export function getConfig(): AgentServerConfig {
  return {
    host: process.env.RALPH_SERVER_HOST ?? process.env.AGENT_SERVER_HOST ?? "localhost",
    port: Number(process.env.RALPH_SERVER_PORT ?? process.env.AGENT_SERVER_PORT ?? 4245),
...
export async function startServer(config: AgentServerConfig): Promise<{
  ...
  app.get("/healthz", (_req: Request, res: Response) => {
    res.json({ ok: true, server: "ralph-server" })
  })
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(config.port, config.host, () => {
      console.log(`[ralph-server] listening on http://${config.host}:${config.port}`)
      resolve()
    })
  })
  ...
  const close = async () => {
    ...
    console.log("[ralph-server] stopped")
  }
