...
| Package                                               | Description                                                                                               |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| [`@herbcaudill/ralph`](packages/ralph-cli/)           | CLI tool for terminal-based sessions                                                                      |
| [`@herbcaudill/ralph-ui`](packages/ralph-ui/)         | Web UI with real-time event streaming                                                                     |
| [`@herbcaudill/ralph-shared`](packages/ralph-shared/) | Shared types and utilities                                                                                |
...
The UI provides real-time monitoring, task management, concurrent worker orchestration, and support for multiple AI agents (Claude, Codex).
...
In split mode, the UI automatically routes API requests and WebSocket connections to the correct server based on path prefixes (`/api/tasks`, `/api/labels`, `/api/workspace` → beads-server; `/api/ralph`, `/api/task-chat`, `/api/instances`, `/api/orchestrator`, `/api/workers` → agent-server). On beads v1, beads-server emits WebSocket refresh events from CLI-backed change polling instead of the old daemon mutation stream.
