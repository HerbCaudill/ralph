...
describe("StatusBar", () => {
  ...
  describe("ConnectionStatusIndicator", () => {
    ...
    it("has success color when connected", () => {
      useAppStore.getState().setConnectionStatus("connected")
      render(<StatusBar />)
      expect(screen.getByTestId("connection-status-icon")).toHaveClass("text-status-success")
    })

    it("has warning color when connecting", () => {
      useAppStore.getState().setConnectionStatus("connecting")
      render(<StatusBar />)
      expect(screen.getByTestId("connection-status-icon")).toHaveClass("text-status-warning")
    })

    it("has error color when disconnected", () => {
      useAppStore.getState().setConnectionStatus("disconnected")
      render(<StatusBar />)
      expect(screen.getByTestId("connection-status-icon")).toHaveClass("text-status-error")
    })
  })
  ...
  describe("TokenUsage", () => {
    it("shows 0/0 when no tokens used", () => {
      render(<StatusBar />)
      // Should show token usage even when both input and output are 0 (useful information)
      expect(screen.getByTitle("Token usage (input / output)")).toBeInTheDocument()
      expect(screen.getAllByText("0")).toHaveLength(2) // Both input and output show 0
    })
