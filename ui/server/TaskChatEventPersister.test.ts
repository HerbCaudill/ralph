...
} from "./TaskChatEventPersister"
import type { TaskChatEvent } from "./TaskChatManager"

describe("TaskChatEventPersister", () => {
  let testDir: string
  let persister: TaskChatEventPersister

  /**
   * Create a unique test directory for each test
   */
  beforeEach(async () => {
    testDir = join(tmpdir(), `task-chat-event-test-${randomBytes(8).toString("hex")}`)
    await mkdir(testDir, { recursive: true })
    persister = new TaskChatEventPersister(testDir)
    resetTaskChatEventPersisters()
   ...
   */
  function createTestEvent(overrides: Partial<TaskChatEvent> = {}): TaskChatEvent {
    return {
  ...
  describe("appendEvent", () => {
    ...
    it("creates the .ralph directory if it does not exist", async () => {
      ...
      const content = await readFile(
        join(testDir, ".ralph", "session-task-chat-test-1.jsonl"),
        "utf-8",
      )
  ...
  describe("readEvents", () => {
    ...
    it("reads all events from file", async () => {
      await persister.appendEvent("test-1", createTestEvent({ type: "event-1" }))
      await persister.appendEvent("test-1", createTestEvent({ type: "event-2" }))
    ...
    it("handles invalid JSON lines gracefully", async () => {
      ...
      await writeFile(
        join(testDir, ".ralph", "session-task-chat-test-1.jsonl"),
        '{"type":"valid","timestamp":1000}\ninvalid json line\n{"type":"also-valid","timestamp":2000}\n',
        "utf-8",
      )
    ...
    it("handles empty lines gracefully", async () => {
      ...
      await writeFile(
        join(testDir, ".ralph", "session-task-chat-test-1.jsonl"),
        '{"type":"event-1","timestamp":1000}\n\n\n{"type":"event-2","timestamp":2000}\n',
        "utf-8",
      )
  ...
  describe("readEventsSince", () => {
    ...
    it("returns all events if timestamp is 0", async () => {
      await persister.appendEvent("test-1", createTestEvent({ type: "event-1", timestamp: 1000 }))
      await persister.appendEvent("test-1", createTestEvent({ type: "event-2", timestamp: 2000 }))

      const events = await persister.readEventsSince("test-1", 0)

      expect(events).toHaveLength(2)
    })

    it("returns empty array if no events after timestamp", async () => {
      await persister.appendEvent("test-1", createTestEvent({ type: "old", timestamp: 1000 }))

      const events = await persister.readEventsSince("test-1", 5000)

      expect(events).toEqual([])
    })

    it("returns empty array for non-existent instance", async () => {
      const events = await persister.readEventsSince("non-existent", 0)
      expect(events).toEqual([])
    })
  })
  ...
  describe("clearAll", () => {
    it("clears all task chat event files", async () => {
      await persister.appendEvent("test-1", createTestEvent())
    ...
    it("handles non-existent directory", async () => {
      const newPersister = new TaskChatEventPersister(join(testDir, "new-workspace"))
      await newPersister.clearAll() // Should not throw
    })

    it("does not affect other file types in .ralph directory", async () => {
      await persister.appendEvent("test-1", createTestEvent())

      // Create a non-task-chat file in .ralph
      await writeFile(join(testDir, ".ralph", "other-file.txt"), "some content", "utf-8")

      await persister.clearAll()

      // Other file should still exist
      const otherContent = await readFile(join(testDir, ".ralph", "other-file.txt"), "utf-8")
      expect(otherContent).toBe("some content")
    })
  })
  ...
  describe("getTaskChatEventPersister factory function", () => {
    it("returns the same persister for the same workspace path", () => {
      const p1 = getTaskChatEventPersister(testDir)
      const p2 = getTaskChatEventPersister(testDir)
      expect(p1).toBe(p2)
    })

    it("returns different persisters for different workspace paths", async () => {
      const testDir2 = join(tmpdir(), `task-chat-event-test-2-${randomBytes(8).toString("hex")}`)
      await mkdir(testDir2, { recursive: true })

      try {
        const p1 = getTaskChatEventPersister(testDir)
        const p2 = getTaskChatEventPersister(testDir2)
  ...
  describe("persistence scenarios", () => {
    it("simulates page reload by reading from disk", async () => {
      ...
      await persister.appendEvent("instance-1", createTestEvent({ type: "tool_use", tool: "Read" }))

      // Simulate page reload - create new persister instance
      const newPersister = new TaskChatEventPersister(testDir)
      const restored = await newPersister.readEvents("instance-1")
    ...
    it("handles completed task chat cleanup", async () => {
      // Write events during task chat
      await persister.appendEvent("instance-1", createTestEvent())
      await persister.appendEvent("instance-1", createTestEvent())

      // Task chat cleared - delete events
      await persister.clear("instance-1")
    ...
    it("supports reconnection sync with readEventsSince", async () => {
      // Server has accumulated events
      await persister.appendEvent(
        "instance-1",
        createTestEvent({ type: "event-1", timestamp: 1000 }),
      )
      await persister.appendEvent(
        "instance-1",
        createTestEvent({ type: "event-2", timestamp: 2000 }),
      )
      await persister.appendEvent(
        "instance-1",
        createTestEvent({ type: "event-3", timestamp: 3000 }),
      )
      await persister.appendEvent(
        "instance-1",
        createTestEvent({ type: "event-4", timestamp: 4000 }),
      )
