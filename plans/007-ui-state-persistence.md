...
- UI view state (`viewingSessionIndex`, `taskSearchQuery`, `selectedTaskId`)
...
- **Active instance events**: Current session only (~100KB), enables instant event stream display

**NO (too large or transient):**

- All instances' full event history - Keep in IndexedDB (too large)
- `tokenUsage`, `contextWindow`, `session` - Runtime metrics, rebuilt on load
...
```typescript
function serializeInstances(instances: Map<string, RalphInstance>, activeInstanceId: string) {
  const array = []
...
function deserializeInstances(array: any[]): Map<string, RalphInstance> {
  ...
  for (const item of array) {
    const instance = createRalphInstance(item.id, item.name, item.agentName)
...
1. Open app, set various UI state (sidebar width, viewing session, search query, etc.)
...
2. Search for "test", select a task, view session 3
...
3. **Instant event stream** - Current session's events show immediately (no IndexedDB wait)
