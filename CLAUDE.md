...
Ralph session task IDs are resolved client-side: `sessionTaskIdCache.ts` caches `sessionId -> taskId` mappings in localStorage (`ralph:sessionTaskIds`). On first load, `fetchRalphSessions` fetches events for uncached sessions, extracts the task ID via `extractTaskLifecycleEvent`, and caches it. Streaming events also populate the cache in `useSessionEvents` when a `start_task` marker is detected.
